...
class SentimentGenerator(NlpTask):
    ...
    async def agenerate(
        ...
        dimensions = dimensions or await self._generate_dimensions_async(concept)
        aspect_map = await self._resolve_aspects_async(concept, dimensions, aspects)
        batch_defs = self._compose_batches(concept, dimensions, aspect_map)
        sentence_objs = await self._generate_sentences_async(batch_defs)
        parsed_rows = self._merge_and_parse_batches(batch_defs, sentence_objs)
        return self._convert_to_output(parsed_rows, SentimentResponse)
    ...
    async def _generate_dimensions_async(self, concept: str) -> List[str]:
        chain = StandardChainBuilder(
            prompt_template=self.config.dimension_prompt,
            llm=self.config.llm,
            entity_model=Dimensions
    ...
    async def _resolve_aspects_async(self, concept: str, dimensions: List[str], aspects: Optional[List[str]]) -> Dict[str, List[str]]:
        if aspects is not None:
            return {dim: aspects for dim in dimensions}
        return await self._generate_aspects_async(concept, dimensions)
    ...
    async def _generate_aspects_async(self, concept: str, dimensions: List[str]) -> Dict[str, List[str]]:
        chain = StandardChainBuilder(
            prompt_template=self.config.aspect_prompt,
            llm=self.config.llm,
            entity_model=Aspects
