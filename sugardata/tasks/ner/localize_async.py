...
class NERLocalizerAsync(NlpTask):
    ...
    async def generate(self, examples: List[Dict[str, Any]], target_language: str) -> NerOutput:
        label_mapping = await self._assign_labels_to_entities(examples)

        batches = await self._compose_batches(examples, target_language)

        generated_text_result = await self._generate_text(batches)
        remap_label_entity = await self._remap_labels_to_entities(
            generated_text_result, label_mapping, examples)

        results = await self._tokenize_and_label(
        ...
        output = await self._convert_to_output_async(parsed_data, NERResponse)
        return output

    async def _assign_labels_to_entities(self, examples: List[Dict[str, Any]]) -> Dict[str, Dict[str, int]]:
        types = []
        for ex in examples:
    ...
    async def _compose_batches(self, examples: List[Dict[str, Any]], target_language: str) -> List[Dict[str, Any]]:
        batches = []
    ...
    async def _generate_text(self, batches: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _remap_labels_to_entities(
            self,
